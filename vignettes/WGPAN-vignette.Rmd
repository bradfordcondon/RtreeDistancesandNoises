...
for (i in loop) {  
  x <- myData #reset matrix
  for (a in as.character(clade_list$ID)) {
    #lookup this clade
  thisClade <-  as.character(unlist(clade_list %>% filter(ID == as.character(a)) %>% select (clades )))
   ...
   matchingTaxa <- as.list(clade_list %>% filter(clades == thisClade) %>% select (ID) %>% filter(ID != as.character(a)))
