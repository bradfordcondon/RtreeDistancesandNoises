...
#output: A list of two (or three) dataframes.  The first documenting each clade's average in and out distances for each tree.  The second is a simplified, average in/out score for each tree. 
...
computeQuantiles <- function (treeDistanceData, cutOffNum, greaterThan = TRUE){
    ...
    #  meltedDataframe =  
  #return (meltedDataFrame)  
}

##
#AnalyzeTips
#goal: determine which taxa are commonly missing from tips
#determine distribution of tip #'s so trees can be filtered.
##
...
#Function: treesToCladeCompareSpecific.  
#input: Multiphylo object, list of clade assignments, list of clades to focus on
#output: A list of two (or three) dataframes.  The first documenting each clade's average in and out distances for each tree.  The second is a simplified, average in/out score for each tree. 
#too convoluted- abandoned for now.
treesToCladeCompareKey <- function(multiPhyloTrees, cladeListFile = "list.txt", referenceTree = NULL, refCompareMethod = "edgeset", keyClades = NULL) {
  ...
  for (b in loop) {
    ...
    for (i in clade_loop){
      ...
      if (is.null(keyClades == FALSE)){ #if running keyclades routine...
       if (i %in% keyClades){#if this clade is a key clade
  ...
  if (is.null(keyClades) == FALSE ){ #add key clades info to list
